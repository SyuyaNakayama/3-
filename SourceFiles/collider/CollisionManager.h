...
private:
	...
	static std::list<PolygonCollider*> triangleColliders_;
	...
	static bool CheckCollisionRayTriangle(RayCollider* colliderA, PolygonCollider* colliderB, float* distance = nullptr, Vector3* inter = nullptr);
	...
	static void CheckCollisions(std::list<RayCollider*> rayColliders, std::list<PolygonCollider*> triangleColliders);
public:
	...
	static void PushCollider(PolygonCollider* collider) { triangleColliders_.push_back(collider); }
	...
	static void PopCollider(PolygonCollider* collider) { triangleColliders_.remove(collider); }
